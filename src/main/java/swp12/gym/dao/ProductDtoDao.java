...
public class ProductDtoDao {
    ...
    public int getNumberProductInSystemAdmin(int status_num) {
        sql = "SELECT COUNT(*) FROM product p join unit u\n" +
                "                        on p.unit_id = u.unit_id\n" +
                "                               join kind k\n" +
                "                         on p.kind_id = k.kind_id\n" +
                "                                join price p2\n" +
                "                        on p.product_id = p2.product_id and p2.status = 1 and p.status = ?";
    ...
    public List<ProductDto> findListProduct(int pagination_value, int status_num) {
        ...
        sql = "SELECT p.product_id as p_id,\n" +
                ...
                "                         on p.kind_id = k.kind_id\n" +
                "                                join price p2\n" +
                "                        on p.product_id = p2.product_id and p2.status = 1 and p.status = ? LIMIT ?,8";

        return jdbcTemplate.query(sql,new ProductDtoMapper(), status_num, pagination_value);
    }
    ...
    public List<ProductDto> searchProductAdmin(String query) {
        try {
            sql = "SELECT p.product_id as p_id,\n" +
                    ...
                    "                         on p.kind_id = k.kind_id\n" +
                    "                                join price p2\n" +
                    "                        on p.product_id = p2.product_id and p2.status = 1 WHERE p.name LIKE '%"+query+"%';";
            return jdbcTemplate.query(sql, new ProductDtoMapper());
        }catch (Exception e) {
            e.printStackTrace();
            return null;
